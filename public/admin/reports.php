...
function csv_download(string $filename, array $headers, array $rows, string $report_type = 'seniors'): void {
	...
	// Add professional header information with logos
	fputcsv($out, ['=' . str_repeat('=', 80)]);
	fputcsv($out, ['']);
	fputcsv($out, ['[DILG LOGO]                    REPUBLIC OF THE PHILIPPINES                    [MANOLO FORTICH LOGO]']);
	fputcsv($out, ['']);
	fputcsv($out, ['REPUBLIC OF THE PHILIPPINES']);
	fputcsv($out, ['PROVINCE OF BUKIDNON']);
	fputcsv($out, ['MUNICIPALITY OF MANOLO FORTICH']);
	fputcsv($out, ['']);
	fputcsv($out, [$main_title]);
	fputcsv($out, ['As of ' . date('F j, Y')]);
	fputcsv($out, ['Control Number: Manolo Fortich-' . date('Y') . '-' . str_pad(rand(1, 999), 3, '0', STR_PAD_LEFT)]);
	fputcsv($out, ['']);
	fputcsv($out, ['=' . str_repeat('=', 80)]);
	fputcsv($out, ['']);
	...
	fputcsv($out, array_fill(0, count($headers), str_repeat('-', 15))); // Separator line
	...
	fputcsv($out, ['Generated on: ' . date('F j, Y \a\t g:i A')]);
	fputcsv($out, ['Total Records: ' . count($rows)]);
	fputcsv($out, ['Generated by: SeniorCare Information System']);
	fputcsv($out, ['Municipality of Manolo Fortich, Bukidnon']);
	fputcsv($out, ['']);
	fputcsv($out, ['=' . str_repeat('=', 80)]);
